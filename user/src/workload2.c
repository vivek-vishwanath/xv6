...
{
    printf(1, "workload_2 test starting\n");
    int fd;
    char buf[512];
    int i;
    int j;
    ...
    // Create 4 small files with 100 blocks each, and 1 big file with 500 blocks, then write to them in order SMALL SMALL SMALL SMALL BIG. Write in cache-friendly manner; write all the way through to one, then move on. 
    ...
    printf(1, "workload_2 test OK\n");
    exit();
    
    // Create 2 files with 30 blocks each, then alternate writes between the files to force cache eviction

    for (k = 0; k < 10; k++) {
        int fileno = k % 2;
        fd = open("file" + fileno, O_RDWR);
        for (i = 0; i < 30; i++) {
            for (j = 0; j < 512; j++) {
                buf[j] = k;
            }
